...
check_prerequisites() {
    ...
    if ! command -v php &>/dev/null; then
        ...
        echo "Please install PHP before proceeding."
        exit 1
    fi
    ...
    if [ ${#missing_extensions[@]} -gt 0 ]; then
        echo "Error: The following required PHP extensions are missing: ${missing_extensions[*]}"
        echo "Please install the missing extensions before proceeding."
        exit 1
    else
        echo "✓ All required PHP extensions (OpenSSL, mbstring, cURL) are installed."
    fi
...
fetch_tags() {
    echo "Fetching available versions from GitHub..." >&2  # Print to stderr to avoid mixing with output
...
install_cypht() {
    ...
    if [ -d "$destination" ]; then
        echo "Cypht version $version already exists at $destination."
    ...
    if [[ "$selected_version$" =~ ^1\.[0-9]+\.[0-9]+$ ]]; then
        echo "Detected version $selected_version. Creating hm3.ini from hm3.sample.ini"
        cp hm3.sample.ini hm3.ini
    else
        echo "Detected version $selected_version. Creating .env from .env.example...."
        cp .env.example .env
    fi
...
read -p "Enter the version number (e.g. 1 for v1.0.0) [master]: " version_choice
